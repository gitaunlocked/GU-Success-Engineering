<template>
  <div class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
    <div class="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
      ...
      <h2 class="text-xl text-gray-600 text-center mb-6">Setup Super Admin</h2>

      <div v-if="success" class="bg-green-50 border border-green-300 text-green-700 p-4 rounded-lg mb-4">
        ✅ Super Admin created! Redirecting to admin panel...
      </div>
      ...
      <div v-if="alreadyExists" class="bg-yellow-50 border border-yellow-300 text-yellow-700 p-4 rounded-lg mb-4">
        ⚠️ Super Admin already exists. Go to <NuxtLink to="/auth" class="underline font-bold">Login</NuxtLink>.
      </div>

      <div v-if="!success && !alreadyExists">
        <p class="text-sm text-gray-500 mb-4 text-center">
          Sign in with the <strong>gitaunlocked@gmail.com</strong> Google account to set up the Super Admin.
        </p>

        <button @click="setupSuperAdmin" :disabled="loading"
...
onMounted(async () => {
  try {
    ...
    console.log('No admin check needed')
  }
})
...
const setupSuperAdmin = async () => {
  ...
  try {
    ...
    if (user.email !== ADMIN_EMAIL) {
      error.value = `Please sign in with ${ADMIN_EMAIL}. You signed in as ${user.email}.`
      await auth.signOut()
      loading.value = false
      return
    }
